...
pub async fn handle(command: ConfigCommands) -> Result<()> {
    match command {
        ...
        ConfigCommands::Whoami => handle_whoami(),
    }
}
...
fn handle_init() -> Result<()> {
    ...
    println!("{}", "Use 'rpsn config set' to set your credentials".dim());
    Ok(())
}
...
fn handle_get() -> Result<()> {
    ...
    for (name, profile) in &config.profiles {
        ...
        println!("    Token: {}", if profile.api_token.is_empty() {
            ...
            format!("{}***", &profile.api_token[..8.min(profile.api_token.len())]).dim().to_string()
        });
    }
...
async fn handle_whoami() -> Result<()> {
    ...
    print(&response.user, OutputFormat::Human)?;
